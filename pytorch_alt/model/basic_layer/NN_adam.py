...
class NN(object):
    ...
    def optimize_normal(self, loss, params):
        ...
        '''
        # the optimize.
        self.global_step = torch.nn.Parameter(torch.Tensor(0))#tf.Variable(0, name='global_step')
        if self.is_update_lr:
            self.lr = self.update_lr()
        else:
            #self.lr = tf.Variable(self.init_lr, trainable=False)
            self.lr = torch.nn.Parameter(self.init_lr, requires_grad=False)
        self.optimizer = torch.optim.Adam()#tf.train.AdamOptimizer(self.lr)
