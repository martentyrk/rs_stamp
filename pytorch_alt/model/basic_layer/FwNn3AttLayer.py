...
class FwNnAttLayer(object):
    ...
    def __init__(self, edim, active='tanh', stddev=None, params=None, norm_type = 'softmax'):
        ...
        # use torch.zeros as the mean, as pytorch will recreate the shape of the mean
        # mean in tensorflow implementation is 0
        self.wline_ca = wline_att_ca or torch.normal(torch.zeros(self.edim, 1), stddev, [self.edim, 1], stddev=stddev)
    ...
    def count_alpha(self, context, aspect,  output, ctx_bitmap, alpha_adj=None):
        ...
        aspect_3dim = torch.reshape(
            torch.tile(output, (1, time_steps)), 
            (-1, time_steps, self.edim)
        )
        ...
        res_act = activation(res_sum, self.active)

        batch_size = context.size(dim=0)#tf.shape(context)[0]
        w_shp0 = self.wline_ca.size(dim=0)#tf.shape(self.wline_ca)[0]
        w_shp1 = self.wline_ca.size(dim=1)#tf.shape(self.wline_ca)[1]
        
        #w_line_3dim = tf.reshape(
        #    tf.tile(self.wline_ca, [batch_size, 1]),
        #    [batch_size, w_shp0, w_shp1]
        #)
        w_line_3dim = torch.reshape(
            torch.tile(self.wline_ca, (batch_size, 1)),
            (batch_size, w_shp0, w_shp1)
        )
        ...
        #    [-1, time_steps]
        #)
        res_act = torch.reshape(
            torch.matmul(res_act, w_line_3dim),
            (-1, time_steps)
        )
    ...
    def count_alpha2(self, context, aspect,  output, ctx_bitmap, alpha_adj=None):
        ...
        Returns:
            A tensor.  The attention weights of the context.
        '''
        time_steps = context.size(dim=1)#tf.shape(context)[1]
        ...
        #)
        aspect_3dim = torch.reshape(
            torch.tile(aspect, (1, time_steps)),
            (-1, time_steps, self.edim)
        )
        ...
        batch_size = context.size(dim=0)#tf.shape(context)[0]
        w_shp0 = context.size(dim=0) #tf.shape(self.wline_ca)[0]
        w_shp1 = self.wline_ca.size(dim=1)#tf.shape(self.wline_ca)[1]
    ...
    def forward(self, context, aspect, output, ctx_bitmap, alpha_adj=None):
        ...
        '''
        mem_size = context.size(dim=1)#tf.shape(context)[1]
        context = context
        output = output
        aspect = aspect
        # adjust attention
        ...
        vec = torch.matmul(
            torch.reshape(alpha, (-1, 1, mem_size)),
            context
        )
        return vec, alpha
    ...
    def forward_wot_sum(self, context, aspect, output, ctx_bitmap, alpha_adj=None):
        ...
        #    [1, 1, self.edim]
        #)
        alpha_3dim = torch.tile(
            torch.reshape(alpha, (-1, mem_size, 1)),
            (1, 1, self.edim)
        )
