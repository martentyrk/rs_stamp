...
class LinearLayer_3dim():
    ...
    def __init__(
        ...
        if params is None:
            ...
            #    Randomizer.random_normal(self.w_shape),
            #    # tf.random_uniform(self.w_shape, -0.0015, 0.035),
            #    trainable=True
            #)

            self.wline = torch.nn.Parameter(torch.Tensor(Randomizer.random_normal(self.w_shape)), requires_grad=True)
        else:
    ...
    def forward(self, inputs):
        ...
        w_shp0 = self.wline.size(dim=0)#tf.shape(self.wline)[0]
        w_shp1 = self.wline.size(dim=1)#tf.shape(self.wline)[1]
        batch_size = inputs.size(dim=0)#tf.shape(inputs)[0]
        ...
        w_line_3dim = torch.reshape(
            torch.tile(self.wline, (batch_size, 1)),
            (batch_size, w_shp0, w_shp1)
        )
        
        # linear translate
        #res = tf.matmul(inputs, w_line_3dim)  
        res = torch.matmul(inputs, w_line_3dim)
