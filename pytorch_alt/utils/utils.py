...
def softmax_mask(inputs, seq_mask, axis=1):
    ...
    inputs = inputs * seq_mask
    _sum = torch.sum(inputs, axis, keepdim=True) + 1e-9
    return inputs / _sum

def alpha_mask(inputs, seq_mask, axis = 1):
    inputs = inputs.float32
    inputs = inputs * seq_mask
    max_nums = torch.max(inputs, axis, keepdim=True)
    inputs = torch.exp(inputs - max_nums)
...
def sigmoid2_mask(inputs, seq_mask, axis = 1):
    ...
    outputs /= (torch.sum(outputs, axis, keepdim = True) / 2)
    return outputs